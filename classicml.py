...
Exports:
    ...
    mean_reciprocal_rank_score -- weighted mean, over samples, of the
        reciprocal rank of the first positive label under a score
        ranking truncated at k.
...
def mean_reciprocal_rank_score(y_true, y_score, k=None, sample_weight=None) -> float:
    """Return the weighted mean reciprocal rank of the scores.

    ``y_true`` must be a non-empty rectangular list of rows with at least
    two columns whose elements are exactly the integers ``0`` and ``1``
    (booleans are rejected). Unlike the other label-ranking metrics, a
    row may contain no positive label. ``y_score`` must be a list of the
    same shape whose elements are finite values of type exactly ``int``
    or ``float`` (booleans are rejected). ``k`` must be ``None`` -- the
    number of columns ``m`` is then used -- or an exact ``int``
    (booleans are rejected) with ``1 <= k <= m``. ``sample_weight`` must
    be ``None`` -- every sample then weighs ``1.0`` -- or a list with
    ...
    ``(-y_score[i][j], j)`` (ties favor the smaller index) and the first
    ``k`` indices are kept. If the first index in that truncated order
    whose true label is ``1`` sits at position ``r``, counted from ``1``,
    the row score is ``q_i = 1.0 / r``; when none of the kept columns is
    positive the row score is ``0.0``. After validation the weights are
    converted to ``float`` in row order; ``math.fsum`` computes the total
    weight ``W = sum(w_i)`` and the weighted score ``S = sum(w_i * q_i)``
    in row order, and the result is ``S / W``. Overflow or invalid
    operations during the ``W`` summation, a non-finite ``W``, or
    ``W <= 0`` raise ValueError; overflow, invalid operations, and
    non-finite values during the post-validation weight conversion, the
    ...
    for i in range(n):
        ...
        rank = 0.0
        for position, j in enumerate(order, start=1):
            if labels[j] == 1:
                try:
                    rank = 1.0 / position
                except (OverflowError, ValueError, ZeroDivisionError) as exc:
