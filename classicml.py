...
Exports:
    ...
    mean_tweedie_deviance -- weighted mean Tweedie deviance of a
        non-negative real vector and a strictly positive prediction
        vector at a power strictly between 1 and 2.
...
def mean_tweedie_deviance(y_true, y_pred, power=1.5, sample_weight=None) -> float:
    ...
    strictly greater than 1 and strictly less than 2.
    ...
    After validation, the values, the power, and the weights are
    converted to ``float`` in input order. With ``q = float(power)``,
    ``t = y_true_i`` and ``p = y_pred_i``, the per-sample deviance is

        d = 2.0 * (t ** (2.0 - q) / ((1.0 - q) * (2.0 - q))
                   - t * p ** (1.0 - q) / (1.0 - q)
                   + p ** (2.0 - q) / (2.0 - q)).

    In input order, ``math.fsum`` computes the total weight
    ``W = sum(w_i)`` and, from the weighted terms, ``L = sum(w_i *
    d_i)``; the result is ``L / W``. If the total-weight summation
    overflows or is invalid, is non-finite, or ``W`` is less than or
    equal to zero, a ValueError is raised. Overflow or invalid
    operations during the post-validation conversion or arithmetic
    (powers, multiplications, divisions, additions, subtractions,
    ``math.fsum``, or the final division), and non-finite intermediate
    values or results, raise FloatingPointError. An exact zero result
    is normalized to ``0.0``.
    ...
    if not finite or not (1 < power < 2):
        raise ValueError(
            "power must be a finite non-boolean number strictly "
            "between 1 and 2"
        )
