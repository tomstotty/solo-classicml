...
Exports:
    ...
    expected_calibration_error -- weighted expected calibration error of a
        binary probability vector over equal-width bins.
...
def expected_calibration_error(
    ...
    ignored; each retained bin contributes the gap
    ``g = abs(T / W - P / W)``. The result is the ``math.fsum`` of the
    ``W * g`` products over bins in ascending bin-index order, divided by
    the total weight of all samples, with an exact zero normalized to
    ...
    try:
        ...
        raise FloatingPointError(
            "non-finite value encountered during expected-calibration-error"
            " computation"
        ) from exc
    if not math.isfinite(total_weight):
        raise FloatingPointError(
    ...
    for bin_index in sorted(bins):
        ...
        try:
            gap = abs(
                pos_weight / bin_weight - prob_weight / bin_weight
            )
            term = bin_weight * gap
        except (OverflowError, ValueError) as exc:
            raise FloatingPointError(
    ...
    try:
        weighted_gap = math.fsum(gap_terms)
        error = weighted_gap / total_weight
    except (OverflowError, ValueError, ZeroDivisionError) as exc:
        raise FloatingPointError(
            "non-finite value encountered during expected-calibration-error"
            " computation"
