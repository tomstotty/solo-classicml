...
Exports:
    ...
    adjusted_mutual_info_score -- mutual information of two integer
        partitions adjusted against its hypergeometric expectation,
        normalized by the arithmetic mean of their entropies.
...
def adjusted_mutual_info_score(labels_true, labels_pred):
    ...
    the sample count. The mutual information and entropies are computed
    exactly as in :func:`normalized_mutual_info_score`. The expected
    mutual information traverses cells in ascending ``i``, ``j`` order
    and, within each cell, integer ``q`` from ``max(1, a_i + b_j - n)``
    through ``min(a_i, b_j)`` in ascending order; with
    ``P = Fraction(comb(a_i, q) * comb(n - a_i, b_j - q), comb(n, b_j))``
    each contribution is ``float(P) * (q / n) * math.log((n * q) /
    (a_i * b_j))`` and ``EMI`` is their ``math.fsum``. With
    ``d = (H_true + H_pred) / 2 - EMI`` the result is ``1.0`` when
    ``d == 0`` and ``(MI - EMI) / d`` otherwise. An exact zero result is
    normalized to ``0.0``. Overflow, invalid operations, or non-finite
    values during the post-validation ``Fraction`` conversion,
    multiplication, division, ``math.log``, or ``math.fsum`` steps raise
    FloatingPointError.
    """
    n = _check_metric_vectors(labels_true, labels_pred)
    for value in labels_true:
