...
Exports:
    ...
    det_curve -- false positive/false negative rates and thresholds for a
        binary score ranking.
...
def det_curve(y_true, y_score, pos_label=1, sample_weight=None):
    """Compute false positive/false negative rates at score thresholds.
    ...
    converted to ``float``. A threshold ``t`` predicts the positive class
    for every sample whose score is greater than or equal to ``t``;
    samples sharing a score are therefore always grouped together. Each
    ``fpr`` entry divides, by the negative-class total weight, the
    ``math.fsum`` of the weights -- taken in input order -- of the
    negative-class samples predicted positive (false positives); each
    ``fnr`` entry likewise divides, by the positive-class total weight,
    the ``math.fsum`` of the weights of the positive-class samples
    predicted negative (false negatives). An exact zero rate is
    normalized to ``0.0``. Overflow, invalid operations, or non-finite
    intermediate values after validation raise FloatingPointError.

    The return value is ``(fpr, fnr, thresholds)``, three lists of
    floats of equal length. The inputs are not modified. Deterministic:
    ...
    thresholds = []
    for value in sorted(set(y_score)):
        thresholds.append(_roc_float(value))
