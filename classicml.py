...
Exports:
    ...
    mean_poisson_deviance -- weighted mean Poisson deviance of a finite
        non-negative true vector against a finite strictly positive
        predicted vector.
...
def mean_poisson_deviance(y_true, y_pred, sample_weight=None) -> float:
    ...
    ``y_true`` and ``y_pred`` must be non-empty lists of equal length
    whose elements are finite values of type exactly ``int`` or
    ``float`` (booleans are rejected); the true values must be
    non-negative and the predicted values strictly positive.
    ``sample_weight`` must be ``None`` -- every sample then weighs
    ``1.0`` -- or a list of the same length whose elements follow the
    same numeric type rules and are finite and non-negative. Any
    container, length, type, range, or finiteness violation (including
    ``OverflowError`` raised by ``math.isfinite``) raises ValueError.

    After validation, the values and weights are converted to ``float``
    in input order. ``math.fsum`` computes the total weight
    ``W = sum(w_i)``; if that summation raises OverflowError or
    ValueError, is non-finite, or ``W`` is less than or equal to zero, a
    ValueError is raised. For each index in input order the unit
    deviance is ``d_i = 2 * p_i`` when ``t_i == 0`` and
    ``d_i = 2 * (t_i * log(t_i / p_i) - t_i + p_i)`` otherwise;
    ``d_i`` is confirmed finite before ``w_i * d_i`` is computed. In
    input order, ``math.fsum`` computes ``L = sum(w_i * d_i)`` and the
    result is ``L / W``. Except for the total-weight summation, any
    OverflowError, ValueError, or ZeroDivisionError raised by the
    post-validation conversion or arithmetic, and any non-finite value
    produced, raises FloatingPointError. An exact zero result is
    normalized to ``0.0``.
    ...
    for value in y_pred:
        if type(value) not in (int, float):
            raise ValueError(
                "y_pred must contain only finite strictly positive "
                "non-boolean numbers"
            )
        try:
            finite = math.isfinite(value)
