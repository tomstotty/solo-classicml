...
def mean_pinball_loss(y_true, y_pred, alpha=0.5, sample_weight=None) -> float:
    ...
    ``y_true`` and ``y_pred`` must be non-empty lists of equal length
    whose elements are finite values of type exactly ``int`` or
    ``float`` (booleans are rejected). ``alpha`` must be a finite
    value of type exactly ``int`` or ``float`` (booleans rejected) in
    the closed interval ``[0, 1]``. ``sample_weight`` must be
    ``None`` -- every sample then weighs ``1.0`` -- or a list of the
    same length whose elements are finite non-negative values of type
    exactly ``int`` or ``float`` (booleans are rejected). Any
    ...
    After validation, the values and weights are converted to
    ``float`` in input order. ``math.fsum`` computes ``W = sum(w)``;
    an ``OverflowError``/``ValueError`` from that summation, a
    non-finite result, or ``W <= 0`` raises ValueError. For each
    index, with ``d = y_true_i - y_pred_i``, the per-sample loss is
    ``l = alpha * d`` when ``d >= 0`` and ``(alpha - 1.0) * d``
    otherwise. ``math.fsum`` computes ``L = sum(w * l)`` in input
    order and the result is ``L / W``. Overflow, invalid operations
    during the post-validation conversion or arithmetic, and
    non-finite intermediate values or results (including the
    ``math.fsum`` for ``L``) raise FloatingPointError. An exact zero
    ...
    for i in range(n):
        ...
        if not math.isfinite(d) or not math.isfinite(l) or not math.isfinite(term):
            raise FloatingPointError(
                "non-finite value encountered during mean pinball loss"
            )
        loss_terms.append(term)
