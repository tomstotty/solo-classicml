...
def dumps(model):
    ...
    rejected, as are any non-fitted models, objects of unsupported types,
    invalid construction parameters, and malformed or non-finite state.
    ...
    if isinstance(model, LinearRegression):
        try:
            return _dumps_linear(model, "LinearRegression")
        except ValueError:
            raise
        except Exception as exc:
            raise ValueError("invalid LinearRegression state") from exc

    if isinstance(model, LogisticRegression):
        try:
            return _dumps_linear(model, "LogisticRegression")
        except ValueError:
            raise
        except Exception as exc:
            raise ValueError("invalid LogisticRegression state") from exc

    raise ValueError(
        "dumps only supports fitted KMeans, PCA, LinearRegression, and "
...
def _dumps_linear(model, class_name):
    """Serialize a fitted LinearRegression or LogisticRegression.

    The feature count is not stored; it is recovered from the width of
    ``w`` on load.
    """
    if model.w is None or model.b is None:
        raise ValueError(
            "%s must be fitted before dumps is called" % class_name
        )
    lr = model.lr
    l2 = model.l2
    max_iter = model.max_iter
    ...
    if (
        ...
        raise ValueError(
            "%s has invalid construction parameters" % class_name
        )
    w = model.w
    ...
    b_text = _quantize_fixed(model.b)
    return (
        '{"class":"'
        + class_name
        + '","lr":'
...
def _load_linear(pairs, model_class, class_name):
    ...
    if keys != _SERIAL_KEYS_LINEAR:
        raise ValueError(
            "%s JSON must have exactly the serialized keys "
...
def loads(text):
    ...
    from centroid width, the linear models from the width of ``w``.
    The argument is not modified.
    """
