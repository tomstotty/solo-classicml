...
Exports:
    ...
    KMedoids -- deterministic k-medoids clustering around actual sample
        points with greedy farthest-first initialization.
...
class ExtraTreesRegressor:
    """Deterministic extremely-randomized forest of regression trees.

    The forest holds ``n_estimators`` trees grown exactly as
    ``DecisionTreeRegressor(max_depth=max_depth)`` except that at every
    non-leaf node only a random subset of features is searched and each
    searched feature offers a single random threshold. Unlike
    ``RandomForestRegressor``, every tree is grown on all ``n`` training
    samples (no bootstrap draws). Node values are sample-order
    ``math.fsum`` means; a node becomes a leaf when it holds one sample,
    when it reaches ``max_depth``, or when no candidate split strictly
    improves the parent's squared-error loss; the ascending
    ``(loss, feature index, t)`` tie-break follows the decision tree
    rules.

    Randomness comes from a single ``random.Random(seed)`` stream consumed
    in tree order. In left-before-right recursion order, every node that
    is neither a single-sample node nor a depth-limited node consumes
    exactly one ``sorted(rng.sample(range(p), max_features))`` draw --
    where ``p`` is the number of training features -- selecting the
    features searched at that node; this includes nodes that then become
    leaves because no selected feature offers a strict improvement.
    Single-sample and depth-limited nodes consume no sample. For each
    selected feature, its ascending distinct values among the node's
    samples are recorded as ``V``; a constant feature yields no candidate
    and consumes no randomness. Otherwise a single
    ``rng.randrange(len(V) - 1)`` call picks the index ``k`` and the
    threshold is ``V[k]`` (so the maximum value is never a threshold).
    ...
    def _build(self, X, y, indices, depth, width, rng):
        ...
        for j in features:
            ...
            if len(values) < 2:
                ...
                continue
            # One random threshold per selected non-constant feature.
            t = values[rng.randrange(len(values) - 1)]
            left_targets = []
            right_targets = []
            for i in indices:
                if X[i][j] <= t:
...
class KMedoids:
    """Deterministic k-medoids clustering around actual sample points.

    The initial medoids are chosen greedily: medoid position 0 is sample
    index 0, and each later medoid is the not-yet-selected sample whose
    smallest Euclidean distance to an existing medoid is largest (ties
    go to the smallest sample index). Euclidean distance is
    ``d(a, b) = sqrt(math.fsum((a_j - b_j) ** 2))`` with the squared
    terms produced and accumulated in ascending column order.

    Each round assigns every sample, in input order, to the closest
    medoid (ties go to the smallest medoid position) and then
    synchronously replaces each medoid with the cluster member
    minimizing the ``math.fsum`` of its distances to every member, with
    those distances accumulated in member input order (ties go to the
    smallest original sample index); empty clusters keep their previous
    medoid. Rounds stop when the medoid indices do not change, after at
    most ``max_iter`` rounds. ``fit`` returns the fitted model and
    ``predict`` a fresh list of integer medoid positions; neither method
    modifies its input. The same parameters and inputs always give the
    same result.

    Every checked subtraction, squaring, ``math.fsum``, and ``sqrt`` that
    raises ``OverflowError``/``ValueError`` or yields a non-finite value
    raises ``FloatingPointError``. A failed ``fit`` leaves the model
    unfitted.
    """
    ...
    @staticmethod
    def _distance(a, b):
        """Euclidean distance ``sqrt(fsum((a_j - b_j) ** 2))`` with terms
        in ascending column order; overflow, invalid operations, and
    ...
    def _assign(self, row, centers):
        """Return the position of the center closest to ``row``; ties go
        to the smallest center position."""
        best_index = 0
        best_distance = None
        for k in range(len(centers)):
            distance = self._distance(row, centers[k])
            if best_distance is None or distance < best_distance:
                best_distance = distance
                best_index = k
        return best_index

    def _initial_indices(self, X):
        """Greedy initial medoid indices: sample 0 first, then repeatedly
        the unselected sample with the largest minimum distance to the
        medoids already chosen (ties go to the smallest sample index)."""
        n = len(X)
        indices = [0]
        selected = {0}
        for _ in range(1, self.n_clusters):
            ...
            for i in range(n):
                ...
                for center_index in indices:
                    distance = self._distance(X[i], X[center_index])
    ...
    def fit(self, X):
        ...
        center_indices = self._initial_indices(X)
        centers = [list(X[i]) for i in center_indices]

        for _ in range(self.max_iter):
            clusters = [[] for _ in range(self.n_clusters)]
            for i in range(n):
                clusters[self._assign(X[i], centers)].append(i)

            new_indices = []
            for k in range(self.n_clusters):
                members = clusters[k]
                if not members:
                    # Empty clusters keep their previous medoid.
                    new_indices.append(center_indices[k])
                    continue
                best_member = None
                best_cost = None
                for candidate in members:
                    terms = [
                        self._distance(X[candidate], X[member])
            ...
            if new_indices == center_indices:
                break
            center_indices = new_indices
            centers = [list(X[i]) for i in center_indices]

        self.cluster_centers_ = [list(X[i]) for i in center_indices]
        self.medoid_indices_ = list(center_indices)
        self._n_features = width
        return self
...
def dumps(model):
    ...
    ``n_clusters``, ``max_iter``, and ``n_features_in`` are positive
    JSON integers. ``medoid_indices`` is an array of length
    ``n_clusters`` whose entries are pairwise distinct non-negative
    JSON integers, and ``cluster_centers`` is a non-empty rectangular
    ``n_clusters`` by ``n_features_in`` array whose elements have type
    exactly ``int`` or ``float`` (booleans and subclasses rejected) and
    are finite. Each coordinate is quantized to 10 decimal places via
...
def _dumps_kmedoids(model):
    ...
    order; ``class`` is ``"KMedoids"`` and the three numeric fields are
    positive exact JSON integers. ``medoid_indices_`` is a list of
    ``n_clusters`` pairwise distinct non-negative exact integers and
    ``cluster_centers_`` a non-empty rectangular ``n_clusters`` by
    ``n_features_in`` list of rows whose elements have type exactly
    ``int`` or ``float`` (booleans and subclasses rejected), finite for
    floats. Each coordinate is quantized to 10 decimal places via
    ``Decimal(str(v))`` with ROUND_HALF_UP (never through ``float``
    first; negative zero becomes ``0.0000000000``), and the fixed text
    must convert back with ``float`` to exactly the original value.
    """
    ...
    if centers is None or indices is None or n_features is None:
    ...
    if not isinstance(indices, list) or len(indices) != n_clusters:
        raise ValueError(
            "medoid_indices_ must be a list of length n_clusters"
        )
    seen = set()
    for index in indices:
        if type(index) is not int or index < 0:
            raise ValueError(
                "medoid_indices_ must contain non-negative integers"
            )
        if index in seen:
            raise ValueError("medoid_indices_ must be pairwise distinct")
        seen.add(index)

    if not isinstance(centers, list) or len(centers) != n_clusters:
        raise ValueError(
            "cluster_centers_ must be a list of n_clusters rows"
        )
    encoded_rows = []
    for row in centers:
        if not isinstance(row, list) or len(row) != n_features:
            raise ValueError(
                "cluster_centers_ must be rectangular with "
                "n_features_in columns"
            )
        encoded_rows.append(
            "["
            + ",".join(
                _quantize_lasso_number(value, "cluster center coordinate")
                for value in row
            )
            + "]"
        )
    ...
    centers_text = "[" + ",".join(encoded_rows) + "]"
    return (
...
def _dumps_forest_regressor(model, class_name):
    ...
    JSON integer, max_depth is a positive JSON integer or the string
    ``"none"``, and the remaining numeric fields (n_estimators,
    max_features, n_features_in) are positive JSON integers with
    ``max_features <= n_features_in``. The ``trees`` array holds exactly
    n_estimators node trees encoded by
...
def _load_kmedoids(pairs):
    ...
    if not isinstance(indices_node, list) or len(indices_node) != n_clusters:
        raise ValueError(
            "medoid_indices must be an array of length n_clusters"
        )
    indices = [
        _expect_int(value, "medoid_indices element")
        for value in indices_node
    ]
    seen = set()
    for index in indices:
        if index < 0:
            raise ValueError("medoid_indices elements must be non-negative")
        if index in seen:
            raise ValueError("medoid_indices elements must be distinct")
        seen.add(index)

    centers_node = data["cluster_centers"]
    if not isinstance(centers_node, list) or len(centers_node) != n_clusters:
        raise ValueError(
            "number of cluster center rows must equal n_clusters"
        )
    centers = []
    for row in centers_node:
        if not isinstance(row, list) or len(row) != n_features:
            raise ValueError(
                "cluster_centers must be a rectangular n_clusters by "
                "n_features_in array"
            )
        centers.append(
...
def _load_forest_regressor(pairs, cls, class_name):
    keys = tuple(key for key, _ in pairs)
    ...
    class_name_entry = data["class"]
    if not isinstance(class_name_entry, str) or class_name_entry != class_name:
        raise ValueError('class must be "' + class_name + '"')
...
def loads(text):
    ...
    ExtraTreesRegressor from ``n_features_in``,
    ...
    KMedoids is reconstructed from ``n_clusters``, ``max_iter``, and
    ``n_features_in`` (all positive JSON integers) with a fresh copy of
    ...
    if class_entry == "RandomForestRegressor":
        try:
            return _load_forest_regressor(
                pairs, RandomForestRegressor, "RandomForestRegressor"
            )
        except ValueError:
            raise
        except Exception as exc:
    ...
    if class_entry == "ExtraTreesRegressor":
        try:
            return _load_forest_regressor(
                pairs, ExtraTreesRegressor, "ExtraTreesRegressor"
            )
        except ValueError:
            raise
        except Exception as exc:
