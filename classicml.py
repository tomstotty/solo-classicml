...
Exports:
    ...
    coverage_error -- weighted mean, over samples, of the number of
        columns scored at or above the score of the lowest-scoring
        positive label (ties included).
...
def coverage_error(y_true, y_score, sample_weight=None) -> float:
    """Return the weighted mean label coverage of the scores.
    ...
    For row ``i``, ``t_i`` is the minimum score among the positive-label
    columns and ``q_i`` is the number of columns whose score is not less
    than ``t_i``; columns tied at the threshold are all counted. After
    validation the weights are converted to ``float`` in row order;
    ``math.fsum`` computes the total weight ``W = sum(w_i)`` and the
    weighted coverage ``S = sum(w_i * q_i)``, and the result is
    ...
    for i in range(n):
        labels = y_true[i]
        scores = y_score[i]
        threshold = min(scores[j] for j in range(width) if labels[j] == 1)
        coverage = sum(1 for j in range(width) if scores[j] >= threshold)
        row_coverages.append(coverage)

    weighted_terms = []
    for i in range(n):
        try:
            term = weights[i] * row_coverages[i]
        except (OverflowError, ValueError) as exc:
            raise non_finite(exc) from exc
        if not math.isfinite(term):
            raise non_finite(None)
