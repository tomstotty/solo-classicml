...
def median_absolute_percentage_error(y_true, y_pred, sample_weight=None) -> float:
    ...
    non-finite, or ``W`` is less than or equal to zero, a ValueError is
    raised. For each index,
    ``e_i = abs(y_true_i - y_pred_i) / max(abs(y_true_i),
    sys.float_info.epsilon)`` and the items are sorted by ``(e_i, i)``
    ascending. Starting from an empty prefix, the weight prefix is
    recomputed with ``math.fsum`` after each included item (zero-weight
    items are retained); the result is the first ``e_i`` whose prefix
    is greater than or equal to ``W / 2``. A prefix exactly equal to
    ``W / 2`` still takes the current ``e_i`` -- adjacent values are
    never averaged. Overflow, invalid operations during the
    post-validation conversion, subtraction, absolute value, sorting,
    the division, or a prefix summation, and non-finite intermediate
    values raise FloatingPointError. An exact zero result is normalized
    to ``0.0``.
    ...
    for i in range(n):
        try:
            abs_true = abs(t[i])
            abs_diff = abs(t[i] - p[i])
            e = abs_diff / max(abs_true, sys.float_info.epsilon)
        except (OverflowError, ValueError, ZeroDivisionError) as exc:
            raise FloatingPointError(
                "non-finite value encountered during median absolute "
                "percentage error"
