...
def mean_squared_percentage_error(y_true, y_pred, sample_weight=None) -> float:
    ...
    percentage error is
    ``e = (t - p) / max(abs(t), sys.float_info.epsilon)`` (so a zero
    true value uses ``epsilon`` as the denominator). ``math.fsum``
    computes ``W = sum(w)`` and ``L = sum(w * e ** 2)`` in input
    order. When the ``math.fsum`` of ``W`` raises OverflowError or
    ValueError, or ``W`` is non-finite or less than or equal to zero,
    a ValueError is raised; otherwise the result is ``L / W``. Other
