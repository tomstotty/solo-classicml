...
def symmetric_mean_absolute_percentage_error(y_true, y_pred, sample_weight=None) -> float:
    ...
    After validation, the values and weights are converted to
    ``float`` in input order. For each index, with ``t`` the true
    value and ``p`` the prediction, let ``d = abs(t) + abs(p)``; when
    ``d == 0`` the symmetric absolute percentage error is
    ``e = 0.0``, otherwise ``e = 2.0 * abs(t - p) / d``. ``math.fsum``
    computes ``W = sum(w)`` and ``L = sum(w * e)`` in input order.
    When ``math.fsum`` overflows or fails while computing ``W``, or
    ``W`` is not finite or ``W <= 0``, a ValueError is raised. The
    result is ``L / W``. Apart from the validation and the
    total-weight checks above, overflow, invalid operations, or
