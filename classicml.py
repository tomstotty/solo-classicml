...
Exports:
    ...
    davies_bouldin_score -- Davies-Bouldin index of a clustering over a
        finite real matrix and an integer label vector.
...
def multiclass_log_loss(y_true, y_prob, sample_weight=None) -> float:
    ...
    The total weight is the ``math.fsum`` of the converted weights,
    accumulated in input order; an overflow or invalid operation during
    that summation, a non-finite total, or a total less than or equal to
    zero raises ValueError. The result is the quotient of two
    ...
    remaining post-validation conversion, multiplication, logarithm,
...
def davies_bouldin_score(X, labels):
    ...
    order within each cluster. The centroid of a cluster has, per column,
    the ``math.fsum`` of that column over the cluster's samples divided
    by the cluster size. The distance between two points ``a`` and ``b``
    is ``sqrt(math.fsum((a_j - b_j) ** 2))`` with terms accumulated in
    column order. ``S_k`` is the ``math.fsum`` of the distances from the
    samples of cluster ``k`` to its centroid, taken in input order,
    divided by the cluster size, and ``M_kl`` is the distance between
    the centroids of clusters ``k`` and ``l``. If any ``k != l`` has
    ``M_kl == 0``, ValueError is raised. Otherwise, in ascending cluster
    order, ``R_k`` is the maximum of ``(S_k + S_l) / M_kl`` over
    ``l != k`` and the result is ``math.fsum(R_k)`` divided by the
    number of clusters, with an exact zero normalized to ``0.0``.
    Overflow, invalid operations, or non-finite intermediate values
    after validation raise FloatingPointError.
    ...
    def distance(a, b):
        ...
        overflow, invalid operations, and non-finite values raising
        FloatingPointError."""
        terms = []
        for j in range(width):
            try:
                ...
                raise FloatingPointError(
                    "non-finite value encountered during davies-bouldin "
                    "score"
                ) from exc
            if isinstance(diff, float) and not math.isfinite(diff):
                fail()
    ...
    for group in groups:
        ...
        for j in range(width):
            centroid.append(
                checked_mean([X[i][j] for i in group], size)
            )
    ...
    for k in range(n_clusters):
        ...
        scatters.append(
            checked_mean(
                [distance(X[i], centroid) for i in groups[k]],
                len(groups[k]),
            )
        )

    # Centroid distances; d(k, l) == d(l, k) exactly, so each unordered
    # pair is computed once with the smaller cluster index first.
    separations = [[0.0] * n_clusters for _ in range(n_clusters)]
    for k in range(n_clusters):
        for l in range(k + 1, n_clusters):
            m = distance(centroids[k], centroids[l])
            if m == 0:
                raise ValueError(
                    "distinct clusters must not have identical centroids"
                )
            separations[k][l] = m
            separations[l][k] = m
    ...
    for k in range(n_clusters):
        ...
        for l in range(n_clusters):
            if l == k:
                continue
            try:
                ratio = (scatters[k] + scatters[l]) / separations[k][l]
            except (OverflowError, ValueError) as exc:
                raise FloatingPointError(
                    "non-finite value encountered during davies-bouldin "
                    "score"
