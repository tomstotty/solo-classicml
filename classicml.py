...
def dumps(model):
    ...
    ROUND_HALF_UP (negative zero becomes ``0.0000000000``). A tree with
    ``max_depth=None`` emits ``"max_depth":"none"``; leaf nodes emit
    ``"feature":-1,"threshold":0.0000000000,"left":[],"right":[]``. A
    positive ``lr``/``tol`` that quantizes to zero is rejected, as are
    any non-fitted models, unsupported objects, invalid construction
...
def _dumps_tree(model):
    ...
    ``"none"`` when the tree is depth-unconstrained. ``_n_features``
    must be a positive exact int and the root a well-formed node: leaves
    carry no threshold or children and are emitted with ``feature`` -1,
    ``threshold`` 0.0000000000 and empty child arrays; internal nodes
    carry an exact-int feature in ``[0, n_features_in)``, a finite
    threshold quantized literally with ``Decimal(str(v))`` (10 places,
    ROUND_HALF_UP, negative zero normalized), and two node children.
    """
    root = model._root
    ...
    return (
        ...
        + str(n_features)
        + ',"tree":'
        + _dump_tree_node(root, n_features)
        + "}"
    )


def _dump_tree_node(node, n_features):
    ...
    if type(feature) is not int or not 0 <= feature < n_features:
        raise ValueError(
            "internal node feature must be an integer in "
            "[0, n_features_in)"
        )
    ...
    return (
        ...
        + threshold_text
        + ',"left":'
        + _dump_tree_node(node.left, n_features)
        + ',"right":'
        + _dump_tree_node(node.right, n_features)
        + "}"
    )
...
def _load_tree(pairs):
    ...
    if keys != _SERIAL_KEYS_TREE:
        raise ValueError(
            "DecisionTreeClassifier JSON must have exactly the "
...
def _load_tree_node(node, n_features):
    if not isinstance(node, dict) or tuple(node.keys()) != _SERIAL_KEYS_TREE_NODE:
        raise ValueError(
            "tree nodes must be objects with exactly the keys "
    ...
    if feature == -1:
        ...
        if threshold != 0.0:
            raise ValueError(
                "leaf nodes must have threshold 0.0000000000"
            )
        for child in (node["left"], node["right"]):
            if not isinstance(child, list) or len(child) != 0:
                raise ValueError(
                    "leaf nodes must have empty left and right arrays"
                )
        return _DecisionTreeNode(label)
    if feature < 0 or feature >= n_features:
        raise ValueError("feature must be in [0, n_features_in)")
    threshold = _expect_fixed(node["threshold"], "threshold")
