...
Exports:
    ...
    label_ranking_loss -- weighted mean, over samples, of the fraction
        of positive-negative label pairs whose ordering is reversed (a
        positive column scoring below a negative column) or tied by the
        score matrix.
...
def label_ranking_loss(y_true, y_score, sample_weight=None) -> float:
    ...
    ``y_true`` must be a non-empty rectangular list of rows with at least
    two columns whose elements are exactly the integers ``0`` and ``1``
    (booleans are rejected); rows with no positive (``1``) or no
    negative (``0``) label are allowed. ``y_score`` must be a list of
    ...
    For row ``i``, ``P`` is the set of positive-label columns and ``N``
    the set of negative-label columns, each taken in ascending column
    order. If either set is empty the row score is ``q_i = 0.0``;
    otherwise the positive-negative pairs are iterated by ``P`` first
    and then ``N``, contributing ``1.0`` when the positive score is
    strictly below the negative score, ``0.5`` when they are equal, and
    ``0.0`` otherwise, and the row score is that ``math.fsum`` total
    divided by ``|P| * |N|``. After validation the weights are
    ...
    ``math.fsum`` steps and divisions, the weighted multiplication, the
    ``S`` summation, or the final division raise FloatingPointError. An
    ...
    for i in range(n):
        ...
        try:
            pair_total = math.fsum(pair_terms)
        except (OverflowError, ValueError) as exc:
            raise non_finite(exc) from exc
        if not math.isfinite(pair_total):
            raise non_finite(None)
        try:
            row_score = pair_total / (
