...
Exports:
    ...
    brier_score_decomposition -- reliability, resolution, and
        uncertainty components of the Brier score for a binary
        probability vector.
...
def _brier_decomp_float(value):
    """Convert a validated Brier-decomposition probability/weight to
    float; overflow, invalid operations, and non-finite results raise
    FloatingPointError."""
    try:
        result = float(value)
    except (OverflowError, ValueError) as exc:
...
def brier_score_decomposition(
    ...
    """Compute the Brier score decomposition into reliability,
    ...
    ``1.0`` -- or a list of the same length whose elements are finite
    non-negative values of type exactly ``int`` or ``float`` (booleans
    are rejected). The total weight and the positive-class weight must
    each be greater than zero. Any violation (including overflow during
    the finiteness checks) raises ValueError.

    After validation, the probabilities and weights are converted to
    ``float``. In input order, ``math.fsum`` gives the total weight
    ``W`` and the positive-class weight ``T``; ``o = T / W`` is the
    overall positive-class fraction. Each sample falls in bin
    ``b = min(int(p * n_bins), n_bins - 1)``. Bins are visited in
    ascending bin index; within a bin, samples are accumulated in input
    order with ``math.fsum`` to obtain the bin weight ``Wb``, the
    positive-class bin weight ``Tb``, and the weighted probability sum
    ``Pb`` (the ``math.fsum`` of the ``w * p`` products). Bins with
    ``Wb == 0`` are omitted; for each retained bin ``ob = Tb / Wb`` and
    ``pb = Pb / Wb``. In ascending bin index order the components are

    ``reliability = fsum(Wb * (pb - ob) ** 2) / W``,
    ``resolution = fsum(Wb * (ob - o) ** 2) / W``, and
    ``uncertainty = o * (1 - o)``,

    each an exact zero normalized to ``0.0``. Overflow, invalid
    operations during the post-validation conversion or arithmetic,
    division by zero, and non-finite intermediate values or results
    raise FloatingPointError.
    ...
    if sample_weight is None:
        ...
        weights = [_brier_decomp_float(value) for value in sample_weight]

    probs = [_brier_decomp_float(value) for value in y_prob]
    ...
    if not math.isfinite(total_weight) or not math.isfinite(positive_weight):
        raise FloatingPointError(
            "non-finite value encountered during brier-score "
            "decomposition computation"
        )
    ...
    if positive_weight <= 0.0:
        raise ValueError(
            "the positive-class weight must be greater than 0"
        )
    ...
    # Each occupied bin maps to its W, T, and w*p term lists, all kept
    # in input order. A dict avoids allocating n_bins lists (n_bins is
    # unbounded) while still letting bins be visited in index order.
    bins = {}
    last_bin = n_bins - 1
    for i in range(n):
    ...
    for bin_index in sorted(bins):
        ...
        if bin_weight == 0.0:
            continue
        try:
            bin_rate = pos_weight / bin_weight
            bin_prob = prob_weight / bin_weight
            reliability_terms.append(
                bin_weight * (bin_prob - bin_rate) ** 2
            )
            resolution_terms.append(
                bin_weight * (bin_rate - overall_rate) ** 2
            )
        except (OverflowError, ValueError, ZeroDivisionError) as exc:
            raise FloatingPointError(
                "non-finite value encountered during brier-score "
        ...
        if (
            ...
            or not math.isfinite(bin_prob)
            or not math.isfinite(reliability_terms[-1])
            or not math.isfinite(resolution_terms[-1])
        ):
            raise FloatingPointError(
                "non-finite value encountered during brier-score "
                "decomposition computation"
