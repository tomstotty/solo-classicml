...
_TREE_NODE_KEYS = ("label", "feature", "threshold", "left", "right")
...
def dumps(model):
    ...
    ``max_depth``, node ``label``/``feature``) are emitted as JSON
    integers; ``lr``, ``l2``, ``tol``, ``b``, node ``threshold`` and
    every array coordinate is quantized to 10 decimal places with
    ROUND_HALF_UP (negative zero becomes ``0.0000000000``). A positive
    ``lr``/``tol`` that quantizes to zero is rejected, as are any
    non-fitted models, unsupported objects, invalid construction
    parameters, and malformed or non-finite state. The argument is not
    modified.
    """
...
def _dumps_tree(model):
    ...
    ``max_depth`` is emitted as a JSON integer or the string ``"none"``
    when it is None; ``n_features_in`` is a positive JSON integer. Each
    node is an object with the keys ``label``, ``feature``, ``threshold``,
    ``left``, ``right`` in that order: leaves carry ``feature`` -1,
    ``threshold`` 0.0000000000 and empty ``left``/``right`` arrays, while
    internal nodes carry a feature index in ``[0, n_features_in)``, a
    10-decimal threshold and child node objects. Construction parameters
    and fitted state are re-validated; anything malformed raises
    ValueError.
    """
    root = model._root
    if root is None:
    ...
    if type(n_features) is not int or n_features <= 0:
        raise ValueError(
            "DecisionTreeClassifier feature count must be a positive integer"
        )
...
def _encode_tree_node(node, n_features):
    if not isinstance(node, _DecisionTreeNode):
        raise ValueError("tree state must contain only tree nodes")
    label = node.label
    ...
    if feature is None:
        if (
            ...
            raise ValueError("leaf nodes must not carry split state")
        return (
            '{"label":'
            + label_text
    ...
    return (
        ...
        + str(feature)
        + ',"threshold":'
        + _quantize_fixed(node.threshold)
        + ',"left":'
        + _encode_tree_node(node.left, n_features)
        + ',"right":'
        + _encode_tree_node(node.right, n_features)
        + "}"
    )
...
def _load_tree(pairs):
    ...
    root = _decode_tree_node(data["tree"], n_features)
...
def _decode_tree_node(node, n_features):
    if not isinstance(node, dict):
        raise ValueError("tree nodes must be JSON objects")
    if tuple(node.keys()) != _TREE_NODE_KEYS:
        raise ValueError(
            "tree nodes must have exactly the keys label, feature, "
    ...
    if feature == -1:
        ...
        if (
            ...
            raise ValueError("leaf left/right must be empty arrays")
    ...
    result.left = _decode_tree_node(node["left"], n_features)
    result.right = _decode_tree_node(node["right"], n_features)
    return result
...
def loads(text):
    ...
    from its own ``n_features_in``. The argument is not modified.
    """
