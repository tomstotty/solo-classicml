...
class RandomForestRegressor:
    """Deterministic bagged forest of squared-error decision-tree
    regressors.

    The forest holds ``n_estimators`` decision trees, each grown exactly
    as ``DecisionTreeRegressor(max_depth=max_depth)``: each node's value
    is the mean of its targets, computed as a sample-order ``math.fsum``
    divided by the number of samples (the root is at depth 0); thresholds
    are the distinct feature values in ascending order except the maximum
    with ``x <= t`` going left; a candidate's loss is the sum of the two
    sides' ``math.fsum((y - side_mean) ** 2)`` in sample order; only
    losses strictly below the parent's same-formula loss are accepted; the
    best candidate is the first in ascending ``(loss, feature index, t)``
    order; and a node becomes a leaf when it holds a single sample, when
    it reaches ``max_depth``, or when no candidate split improves the
    loss.
    ...
    rows are kept in draw order. Then, at every non-leaf node in
    left-before-right recursion order, ``sorted(rng.sample(range(p),
    max_features))`` -- where ``p`` is the number of training features --
    selects the features searched at that node, and the split is found
    using only those features under the rules above. Leaf nodes consume no
    sample. Prediction averages the leaf values over the trees in tree
    order for each input row: ``math.fsum(values) / n_estimators``, with
    an exact zero normalized to positive 0.0. The same parameters and
    ...
    def _build(self, X, y, indices, depth, width, rng):
        ...
        node.feature = feature
        node.threshold = threshold
        node.left = self._build(X, y, left_indices, depth + 1, width, rng)
        node.right = self._build(X, y, right_indices, depth + 1, width, rng)
        return node
    ...
    def predict(self, X) -> list[float]:
        ...
        for row in X:
            ...
            try:
                value = math.fsum(values) / self.n_estimators
            ...
            results.append(_positive_zero(value))
        return results
